...
The first documented approach to make use of user search patterns was that of Brill and Cucerzan in 2004 \cite{webuserpoweredspelling}. Their implementation compared search engine query logs  with a large corpus to gather candidate corrections and used a context-dependent, weighted, edit distance error model was used to make comparisons. Given that the majority of queries are correct, the transformations can be iteratively applied to arrive at more common (correct) queries. The approach used bi-grams to break down longer queries. A similar approach employed an \textit{Expectation Maximization algorithm} instead of a of corpus corrections \cite{webuser3}, while comparable, it did not perform as well as \textit{Aspell}, a corrector that relies on manually derived data.
